...
def init_greedy(size):
    ...
    for row in range(1,size+1):
        rowCandidate.append(row)
    for col in range(1,size+1):
        choice = random.choice(rowCandidate)
        rowCandidate.remove(choice)
        try:
...
def main():
 for i in range(2):
        ...
        start=datetime.now()  
        result = min_Conflicts(init_greedy(size))
        if result == False:
            print("failed")
        end = datetime.now()
        print("time used : " + str((end-start).total_seconds()) + " seconds")
